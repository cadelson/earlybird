...
pacman::p_load(tidyverse, scales, glitr)

df_23_25 <- read.csv("~/Github/dewormr/Dataout/gwsd_23_25.txt")
...
df_abate %>% 
  ...
  scale_y_continuous(labels=comma, breaks=seq(0, 7500, 2500), limits=c(0,7500))+
  si_style_ygrid()+
  labs(x = NULL, y = NULL)+
  ...
  geom_text(aes(label = comma(value)), position=position_dodge2(width=.9, preserve="single"), size = 4, vjust = -.3, family = "Source Sans Pro", fontface = "italic") 

#df_abate %>% group_by(status) %>% summarise(across(c(value), sum, na.rm=TRUE)) %>% View()

si_save("Images/south_sudan/arm_gr/2025_arm/abate", width = 11)  
...
df_23_25 %>% 
  ...
  scale_y_continuous(breaks=seq(0, 1, 1), limits=c(0, 500))+
  labs(X = NULL, y = NULL)+
  si_style_nolines()+
...
si_save("Images/south_sudan/arm_gr/2025_arm/abate_used", height = 2, width = 11)  
...
# Caclulate for title
df_23_25 %>% 
  filter(indicator %in% c("abate_targeted", "abate_eligible", "abate_treated", "abate_used"),
         year == current_year) %>% 
  summarise(value = sum(value), .by = indicator)
