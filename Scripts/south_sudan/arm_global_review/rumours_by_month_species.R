...
# DATE: Nov 7, 2024
# NOTES: 
pacman::p_load(tidyverse, scales, glitr, readxl, janitor)
...
df_rumours_species <- df_21_25 %>% 
  ...
  mutate(host = case_when(
    host %notin% c("Human", "Cat", "Dog") ~ "Wild Animal",
    TRUE ~ host)) %>% 
  summarise(value = sum(value, na.rm = TRUE), .by = c(year, month, host)) %>% 
  ...
  mutate(
    ...
    endpoints = case_when(
      ...
      TRUE ~ NA_real_),
    host = factor(host, levels = c("Dog", "Cat", "Wild Animal")))
...
si_save("Images/south_sudan/arm_gr/2025_arm/rumours_monthly_species", width = 12)
