...
pacman::p_load(tidyverse, writexl, glitr, scales, waffle)
...
df_cr %>% 
  ...
  geom_text(aes(label=value_lab), position=position_dodge2(width=.7, preserve="single"), na.rm=TRUE, color="black", vjust=-.5, size=3.5, family="Source Sans Pro SemiBold")+
  ...
  scale_y_continuous(breaks=seq(0, 350000, 100000), limits=c(0, 350000), labels=label_number(suffix="k", scale = 1e-3))+
  scale_x_discrete(labels = c("J", "F", "M", "A", "M", "J", "J", "A", "S", "O", "N", "D"))+
  #geom_rug(aes(color = factor(cases_color)), size=2, sides="b", na.rm = TRUE) +
  si_style_ygrid()+
  labs(x = NULL, y = NULL)+
  theme(axis.text.x  = element_text(size = 18, family = "Source Sans Pro"),
        axis.text.y  = element_text(size = 20, family = "Source Sans Pro" ),
        strip.text = element_text(size=24, hjust=.02, vjust=.5, family = "Source Sans Pro"),
...
si_save("Images/south_sudan/arm_gr/2025_arm/cr_month")  
...
df_cr_waf<-df_cr %>% 
  ...
  mutate(percent = cr_reached/sum(cr_reached)) %>% 
  View()
  #group_by(risk_level) %>% 
  ungroup() %>% 
...
ggsave("Images/2025_gr/cr_gr_waffle.png",
       height = 7,
       width = 7)
