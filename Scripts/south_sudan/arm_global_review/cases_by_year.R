...
pacman::p_load(tidyverse, glitr, scales, lubridate, extrafont, AzureStor, cowplot)
...
#si_save("Images/south_sudan/arm_gr/2026_arm/gw_yearly_cases_non_subcutaneous.png")

df_year_small <- df_year %>% 
  filter(year>2015) %>% 
  ggplot(aes(x=year, y=cases))+
  geom_area(alpha=.5, fill=denim_light)+
  geom_vline(xintercept = seq(from = 2016, to = 2024, by = 1), size=.5, color="white", linetype="dotted")+
  ...
  geom_point(fill = "white",
             ...
             stroke=2) +
  si_style_ygrid()+
  scale_y_continuous(labels = number_format(accuracy=1), breaks=seq(from=0, to=20, by=5), limits=c(0, 20))+
  scale_x_continuous(breaks=seq(2016, 2026, 1))+
  labs(x = NULL, y = NULL, color = NULL, subtitle="2016-2025", family="Source Sans Pro")+
  scale_size_area()+
  theme(axis.text = element_text(size = 14),
        plot.subtitle = element_text(size=15, hjust=.15, vjust=3))

#si_save("Images/south_sudan/arm_gr/2026_arm/cases_yearly_recent.png")


ggdraw() +
  draw_plot(df_year_main) +
  draw_plot(df_year_small, x = 0.4, y = 0.55, width = 0.6, height = 0.4)

si_save("Images/south_sudan/arm_gr/2026_arm/cases_yearly_full.png")
...
si_save(paste0("Images/2025_arm/county_presentations/cases/yearly_cases_", 
...
si_save("Images/2025_arm/county_presentations/cases/yearly_cases_ak_ny.png")
