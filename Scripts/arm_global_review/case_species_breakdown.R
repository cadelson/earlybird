...
df_cases_species <- df_cases_global %>% 
  ...
  mutate(
    ...
    host_label = factor(host_label, levels = unique(host_label[order(-cases)])))
...
df_cases_species %>% 
  ...
  theme(axis.text.x  = element_text(vjust = 0.5, size = 34, 
  ...
  geom_rect(data = data.frame(Year = "Serval Cat (n=5)"), xmin = 3.5, xmax = 7.5, ymin = -Inf, ymax = Inf, alpha = 0.2, fill=old_rose_light, inherit.aes = FALSE)+
  scale_fill_identity()
